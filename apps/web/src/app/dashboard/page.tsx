...
export default function DashboardPage() {
  ...
  return <div className="min-h-screen bg-canvas">
    <header className="sticky top-0 z-40 border-b border-black/5 bg-canvas/90 backdrop-blur-xl"><div className="page-container flex h-16 items-center justify-between"><Link href="/" className="focus-ring flex items-center gap-3"><Image src="/logo.png" alt="SADEAN" className="h-9 w-9" /><span className="text-lg font-black tracking-tight text-ink">SADEAN</span></Link><div className="flex items-center gap-3"><span className="text-xs font-bold uppercase tracking-wider text-primary-700">{title}</span><span className="hidden text-sm font-bold text-gray-700 sm:block">{user.name}</span><button onClick={async () => { await logout(); toast.success('Berhasil logout'); router.push('/login'); }} className="focus-ring rounded-xl px-4 py-2 text-sm font-bold text-red-600 hover:bg-red-50">Keluar</button></div></div></header>
