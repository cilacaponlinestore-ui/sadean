  ...
  return <div className="min-h-screen bg-canvas"><Navbar />
    <main id="main-content" className="page-container py-8 sm:py-12">
      ...
      : <div className="mt-8 grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-3 lg:grid-cols-4">{items.map((item) => <div key={item.id} className="group relative overflow-hidden rounded-2xl border border-black/5 bg-white transition hover:-translate-y-1 hover:shadow-soft">
        <Link href={`/products/${item.product.slug}`} className="focus-ring block"><div className="aspect-square overflow-hidden bg-[#eee9df]">{item.product.image ? <Image src={item.product.image} alt={item.product.name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} /> : <div className="flex h-full items-center justify-center text-4xl opacity-30">📦</div>}</div></Link>
        <button aria-label="Hapus dari favorit" onClick={() => remove(item.productId)} className="focus-ring absolute right-2 top-2 z-10 rounded-full bg-white/95 p-2 shadow-soft opacity-100 transition hover:bg-red-50 md:opacity-0 md:group-hover:opacity-100"><svg className="h-5 w-5 text-red-500" viewBox="0 0 24 24" fill="currentColor"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" /></svg></button>
