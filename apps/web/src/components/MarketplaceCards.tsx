  ...
  return (
    <Link href={`/products/${product.slug}`} className="group overflow-hidden rounded-2xl border border-black/5 bg-white transition hover:-translate-y-1 hover:shadow-soft focus-ring">
      ...
      <div className="p-3.5 sm:p-4">
        <p className="text-[11px] font-bold uppercase tracking-wider text-primary-700">{product.category?.name || 'Produk UMKM'}</p>
        <h3 className="mt-1 line-clamp-2 min-h-10 font-extrabold leading-5 text-ink">{product.name}</h3>
        <p className="mt-2 truncate text-xs text-gray-500">{product.seller?.storeName || 'UMKM Cilacap'}</p>
        <p className="mt-2 text-lg font-black text-primary-700">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(Number(product.price))}</p>
      </div>
    </Link>
  );
}
